...
function Web3Selector({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {ethBalance.isError && <span className="text-red-500 text-sm -mt-2 ml-4">Error loading balance</span>}

      {/* Action Panel - shows only when a wallet is selected */}
      {selectedWallet && (
        <div className="flex flex-col gap-3 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700 mt-2">
          <h3 className="font-semibold text-gray-900 dark:text-white">Send Transaction</h3>
          
          <div>
            <label className="text-sm text-gray-700 dark:text-gray-300 block mb-1">Amount (ETH)</label>
            <input type="number" value={ethToSend} className="w-full bg-white dark:bg-gray-700 text-black dark:text-white border border-gray-300 dark:border-gray-600 rounded px-3 py-2" onChange={(e) => setEthToSend(e.target.value)} />
          </div>

          <div>
            <label className="text-sm text-gray-700 dark:text-gray-300 block mb-1">Recipient Address</label>
            <input type="text" value={recipientAddress} className="w-full bg-white dark:bg-gray-700 text-black dark:text-white border border-gray-300 dark:border-gray-600 rounded px-3 py-2" placeholder="0x..." onChange={(e) => setRecipientAddress(e.target.value)} />
          </div>
          
          <button
            className={`mt-2 rounded px-4 py-2 text-white 
              ${!recipientAddress || !ethToSend || busySendingEth ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`}
            disabled={!recipientAddress || !ethToSend || busySendingEth}
            onClick={sendEth}
          >
            {busySendingEth ? 'Sending...' : 'Send ETH'}
          </button>
          
          {sendTxError && <div className="text-red-500 text-sm">Error: {sendTxError.message}</div>}
        </div>
      )}
    </div>
  );
